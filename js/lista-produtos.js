...
function displayItems() {
    ...
    endereco.innerHTML = `
        ...
        <div class="inputs">
			<input type="radio" name="cep" value="3" id="cep">Conchal R$ 3,00 <br>
			<input type="radio" name="cep" value="5" id="cep">Tujuguába R$ 5,00
		</div>
    ...
    iphonesItems.map(item => {
        ...
        itemPrice.innerText = formatarValor(item.price) + ',00 R$';
...
function addToCart() {
    ...
    var cep = document.querySelectorAll("input[name='cep']")
    let finSelect = () => {
        let selected = document.querySelector("input[name='cep']:checked")

        if (selected.value == '3') {
            document.getElementById('total-price').innerText = 'VALOR TOTAL: ' + formatarValor(sum + 3) + ',00 R$';
        } else {
            document.getElementById('total-price').innerText = 'VALOR TOTAL: ' + formatarValor(sum + 5) + ',00 R$';
        }
    }

    cep.forEach(cepTodos => {
        cepTodos.addEventListener('change', finSelect)
    })
...
let finSelect = () => {
    ...
    if (selected.value == '3') {
        cpValidacao.push('3,00 R$ - Conchal')
    } else {
        cpValidacao.push('5,00 R$ - Tujuguába')
    }
}
...
`
    ...
    var FreteConfrimacao = cpValidacao.pop()
    
    Menssagemwhatt(rua, Bairro, Numero, valuesect, valueselector, dataAtual, horarioAtual, valores, preco, troco, FreteConfrimacao)
